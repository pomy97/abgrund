# @s = marker
data merge entity @s {Tags:["abg.this","abg.spawnpoint"]}

$execute store success storage abgrund:perma_storage spawn_safe byte 1 in abgrund:limbo run \
    spreadplayers $(X) $(Z) 1 5 false @s
execute store result storage abgrund:perma_storage test_spawn.X int 1 run data get entity @s Pos[0]
execute store result storage abgrund:perma_storage test_spawn.Z int 1 run data get entity @s Pos[2]
